...
import com.maoyou.springframework.core.io.FileSystemResource;
import com.maoyou.springframework.core.io.Resource;
import com.maoyou.springframework.core.io.ResourceLoader;
import com.maoyou.springframework.core.io.UrlResource;
...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
                ...
                for (URL url : ((URLClassLoader) classLoader).getURLs()) {
                    try {
                        UrlResource jarResource = (ResourceUtils.URL_PROTOCOL_JAR.equals(url.getProtocol()) ?
                                new UrlResource(url) :
                                new UrlResource(new URL(ResourceUtils.JAR_URL_PREFIX + url + ResourceUtils.JAR_URL_SEPARATOR)));
                        if (true) {
        ...
        try {
            ...
            for (String path : StringUtils.delimitedListToStringArray(
                ...
                try {
                    ...
                    UrlResource jarResource = new UrlResource(new URL(ResourceUtils.JAR_URL_PREFIX +ResourceUtils.FILE_URL_PREFIX + filePath + ResourceUtils.JAR_URL_SEPARATOR));
                    // Potentially overlapping with URLClassLoader.getURLs() result above!
                    if (!result.contains(jarResource) && !hasDuplicate(filePath, result) && true) {
                        result.add(jarResource);
                    }
                }
                catch (MalformedURLException ex) {
    ...
    private boolean hasDuplicate(String filePath, Set<Resource> result) {
        ...
        try {
            return result.contains(new UrlResource(new URL(ResourceUtils.JAR_URL_PREFIX + ResourceUtils.FILE_URL_PREFIX +duplicatePath + ResourceUtils.JAR_URL_SEPARATOR)));
        }
        catch (MalformedURLException ex) {
