...
import com.maoyou.springframework.beans.factory.annotation.ContextAnnotationAutowireCandidateResolver;
...
//                ClassPathBeanDefinitionScanner scanner = new ClassPathBeanDefinitionScanner(this.getRegistry(), this.getResourceLoader());
        ...
        while (current != null) {
            ...
            for (Method method : methods) {
                ...
                if (methodName.startsWith("set")) {
                    String fieldName = methodName.substring(3).substring(0, 1).toLowerCase() + methodName.substring(3).substring(1);
    ...
    private void registerComponents(Set<BeanDefinition> beanDefinitions) {
        boolean annotationConfig = true;
        if (annotationConfig) {
            DefaultListableBeanFactory beanFactory = unwrapDefaultListableBeanFactory(super.getRegistry());
