...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader implements BeanDefinitionReader {
    ...
    private void doLoadBeanDefinitions(InputStream inputStrem) throws DocumentException {
        ...
        while (it.hasNext()) {
            ...
            if (StringUtils.hasLength(bean.attributeValue("init-method"))) {
                bd.setInitMethodName(bean.attributeValue("init-method"));
            }
            if (StringUtils.hasLength(bean.attributeValue("destroy-method"))) {
                bd.setDestroyMethodName(bean.attributeValue("destroy-method"));
            }
            if (StringUtils.hasLength(bean.attributeValue("scope"))) {
                bd.setScope(bean.attributeValue("scope"));
            }
                ...
                if ("property".equals(e.getName()) && e.attributeValue("name") != null && !"".equals(e.attributeValue("name"))) {
                    ...
                    if (e.attributeValue("ref") != null && !"".equals(e.attributeValue("ref"))) {
                        ...
                        if ("true".equals(value)) {
                            pv = new PropertyValue(e.attributeValue("name"), true);
                        } else if ("false".equals(value)) {
                            pv = new PropertyValue(e.attributeValue("name"), false);
                        } else {
            ...
            getRegistry().registerBeanDefinition(bean.attributeValue("id") != null ? bean.attributeValue("id") : bd.getBeanClassName(), bd);
        }
    }
